...
export const ConnectionRenderer = {
    ...
    drawActiveLine(ctx, fromNode, mouseWorldPos, camera) {
        ctx.save();
        try {
            // Updated: Use getEdgePoint to support both Circles and Rectangles (Containers)
            // This ensures the preview line starts from the edge, not center, and respects the shape.
            const startPoint = GeometryUtils.getEdgePoint(
                fromNode,
                mouseWorldPos.x,
                mouseWorldPos.y,
                null, // nodes array not strictly needed for single-node edge calc in this context
                camera
            );
