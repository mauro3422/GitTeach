...
export class ResilienceAggregator extends IMetricAggregator {
    ...
    aggregate(nodes, totalFiles = 0) {
        ...
        nodes.forEach(node => {
            ...
            if (res.error_discipline !== undefined) {
                totals.error_discipline += res.error_discipline || 0;
                totals.defensive_posture += res.defensive_posture || 0;
                totals.optimization_score += res.optimization_score || 0;
    ...
    validate(nodes) {
        ...
        // Check if any nodes have resilience forensics data
        const hasResilienceData = nodes.some(node =>
            node.metadata?.resilience_forensics &&
            typeof node.metadata.resilience_forensics === 'object'
        );

        return hasResilienceData;
    }
}
